...
//  WakeUpAt
//
//  Created by Gaurav Bidalia on as.
...
class AlarmSettingsVC: UIViewController, AlarmSettingsTableVCDelegate {
    ...
    fileprivate func setupViewProperties() {
        if alarm == nil {
            navigationItem.title = "Add Trigger"
            saveButton.isEnabled = false
            deleteButton.isHidden = true
        }
        else {
            navigationItem.title = "Edit Trigger"
        }
    }
    
    func showDeleteAlarmAlert() {
        let alert = UIAlertController(title: "Delete Trigger", message: "Are you sure you want to delete this trigger?", preferredStyle: .alert)
